...
for i=1:5
sigma(i)=asin(H/D_jing(i));%%目标高低角，与目标距离和深度有关
    for j=1:361        %%从-180到180共有361个数据
if (-90<=q1(j))&&(q1(j)<=90)%%当目标舷角度处于-90°到90°之间时
%%D1(i)=((Cs^2-V^2)*T(j))/2/(Cs+V*cos(q(j))*cos(deita(i)));
T(j)=2*D_jing(i)*(Cs+V*cos(q(j))*cos(sigma(i)))/(Cs^2-V^2);%%T为声波发出到接收的总时间，受目标舷角与目标距离的影响
t_jian(j)=Cs*T(j)/2/(Cs+V*cos(q(j))*cos(sigma(i)));
D_jian(j)=Cs^2*T(j)/2/(Cs+V*cos(q(j)));
else
%%D1(i)=((Cs^2-V^2)*T(j))/2/(Cs-V*cos(q(j))*cos(deita(i)));
T(j)=2*D_jing(i)*(Cs-V*cos(q(j))*cos(sigma(i)))/(Cs^2-V^2);
t_jian(j)=Cs*T(j)/2/(Cs-V*cos(q(j))*cos(sigma(i)));
D_jian(j)=Cs^2*T(j)/2/(Cs-V*cos(q(j)));
end
...
figure;plot(q1,D_error2);legend('100m','200m','300m','400m','500m');ylabel('目标距离计算误差（m）');xlabel('目标舷角（°）');title('工程模型相比精确模型的距离计算误差')
figure;plot(q1,t_error2);legend('100m','200m','300m','400m','500m');ylabel('反射段时间计算误差（s）');xlabel('目标舷角（°）');title('工程模型相比精确模型的反射时间计算误差')